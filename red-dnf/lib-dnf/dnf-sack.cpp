...
/**
 * dnf_sack_add_rpmdb_repo:
 * @sack: a #DnfSack instance.
 * @reponame: a reponame to separate coreos,template,profile and project RPMdb
...
dnf_sack_add_rpmdb_repo(DnfSack *sack, const char* reponame, GError **error) try
{
    ...
    // if not provided create a dedicate hyrepo to load rpmdb
    HyRepo hrepo = hy_repo_create(reponame);
    auto repoImpl = libdnf::repoGetImpl(hrepo);

    // retreive session libsolv existing repo instance from pool
    DnfSackPrivate *priv = GET_PRIVATE(sack);
    Pool *pool = dnf_sack_get_pool(sack);
    ...
    if (cache_fp)
    {
        status = current_rpmdb_checksum(pool, repoImpl->checksum);
        if (status != 0)
            g_warning("failed calculating RPMDB checksum");
    }

    //printf("dnf_sack_add_rpmdb_repo fetching rpmdb rootdir=%s reponame=%s\n", pool_get_rootdir(pool), reponame);
    if (status==0 && can_use_rpmdb_cache(cache_fp, repoImpl->checksum))    {
        const char *chksum = pool_checksum_str(pool, repoImpl->checksum);
        g_debug("using cached rpmdb path=%s checksum=(0x%s)", cache_fn, chksum);
        status = repo_add_solv(repo, cache_fp, 0);
        if (status != 0)
            repoImpl->state_main = _HY_LOADED_CACHE;
    } else {
        g_debug("fetching rpmdb rootdir=%s reponame=%s", pool_get_rootdir(pool), reponame);
        int flagsrpm = REPO_REUSE_REPODATA | RPM_ADD_WITH_HDRID | REPO_USE_ROOTDIR;
        status = repo_add_rpmdb_reffp(repo, cache_fp, flagsrpm);
        if (status != 0)
            repoImpl->state_main = _HY_LOADED_FETCH;
    }
    ...
    if (repoImpl->state_main == _HY_LOADED_FETCH) {
        status = write_main(sack, hrepo, 1, error);
        if (status != 0)
            goto finish;
    }
